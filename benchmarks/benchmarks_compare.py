...
analysis.write("project,criteria,slc4jold-instrexec,slc4jold-slicing,slc4jold-slice-n,slc4jnew-instrexec,slc4jnew-slicing,slc4jnew-slice-n,js-instrexec,js-slicing,js-slice-n\n")
...
def run_slicer4jold(project, jar_name, project_arg, extra_libs, sc_file, slice_line):
    slice_file = "slice-result.log"
    cwd = os.getcwd()
    out_dir=f"results/{project}_old"
    start_instr = time.time()
    if os.path.isdir(out_dir):
        os.system(f"rm -r {out_dir}")
    os.mkdir(out_dir)
    instr_cmd = f"java -cp \"{slicer4j_dir}/target/slicer4j_old.jar:{slicer4j_dir}/target/lib/*\" ca.ubc.ece.resess.slicer.dynamic.slicer4j.Slicer -m i -j {jar_name} -o {out_dir}/ -sl {out_dir}/static-log.log -lc {dynamic_slicing_core}/DynamicSlicingLoggingClasses/DynamicSlicingLogger.jar > /dev/null 2>&1"
    os.system(instr_cmd)
    ...
    analysis.write(str(run_time-start_instr))
    analysis.write(",")
    analysis.write(str(slice_time-run_time))
    analysis.write(",")
    analysis.write(str(num_lines))
    analysis.write(",")

def run_slicer4jnew(project, jar_name, project_arg, extra_libs, sc_file, slice_line):
    slice_file = "slice-result.log"
    cwd = os.getcwd()
    ...
    if os.path.isdir(out_dir):
        os.system(f"rm -r {out_dir}")
    os.mkdir(out_dir)
    instr_cmd = f"java -cp \"{slicer4j_dir}/target/slicer4j_new.jar:{slicer4j_dir}/target/lib/*\" ca.ubc.ece.resess.slicer.dynamic.slicer4j.Slicer -m i -j {jar_name} -o {out_dir}/ -sl {out_dir}/static-log.log -lc {dynamic_slicing_core}/DynamicSlicingLoggingClasses/DynamicSlicingLogger.jar > /dev/null 2>&1"
    os.system(instr_cmd)
    instr_time = time.time()
    print(f"Instrumentation time (s): {instr_time-start_instr}")
    instrumented_jar = os.path.basename(jar_name).replace(".jar", "_i.jar")
...
def run_javaslicer(project, jar_name, project_arg, extra_libs, sc_file, slice_line, slice_method):
    ...
    print(f"Slice size (Java LoC): {num_lines}")
    analysis.write(str(exec_time-start))
    analysis.write(",")
    analysis.write(str(slice_time-exec_time))
    analysis.write(",")
    analysis.write(str(num_lines))
    analysis.write(",")
...
#for idx, project in enumerate(benchmarks_input):
    ...
    #analysis.write(project)
    #analysis.write(",")
    #analysis.write(str(slice_line))
    #analysis.write(",")
    #print(f"====================")
    ...
    #print(f"********************")
    #print(f"Running Slicer4J (old)")
    #run_slicer4jold(project, jar_name, project_arg, "", "", "")
    #print(f"********************")
    #print(f"Running Slicer4J (new)")
    #run_slicer4jnew(project, jar_name, project_arg, "", "", "")
    #print(f"********************")
...
for idx, project in enumerate(defects4j_benchmarks):
    ...
    print(f"********************")
    print(f"Running Slicer4J (old)")
    #run_slicer4jold(project, jar_name, project_arg, extra_libs, sc_file, slice_line)
    print(f"********************")
    print(f"Running Slicer4J (new)")
    run_slicer4jnew(project, jar_name, project_arg, extra_libs, sc_file, slice_line)
    print(f"********************")
    print(f"Running JavaSlicer")
